...
export class Excel {
    ...
    UPPER = (x, y) => {
        const cell = this.grid[y][x];
        if (typeof cell.value === 'string') {
            cell.setValue(cell.value.toUpperCase());
        }
    }
    ...
    LOWER = (x, y) => {
        const cell = this.grid[y][x];
        if (typeof cell.value === 'string') {
            cell.setValue(cell.value.toLowerCase());
        }
    }

    /**
     * Removes duplicate rows from a selected range.
     *
     * @param {number} startX   Starting X-axis of the range.
     * @param {number} startY   Starting Y-axis of the range.
     * @param {number} endX     Ending X-axis of the range.
     * @param {number} endY     Ending Y-axis of the range.
     */
    REMOVE_DUPLICATES = (startX, startY, endX, endY) => {
        const uniqueRows = new Set();
        for (let y = startY; y <= endY; y++) {
            const row = this.grid[y].slice(startX, endX + 1).map(cell => cell.value).join(',');
            if (uniqueRows.has(row)) {
                this.grid[y].forEach(cell => cell.setValue(''));
            } else {
                uniqueRows.add(row);
            }
        }
    ...
    /**
     * Allows users to find and replace specific text within a range of cells.
     *
     * @param {number} startX   Starting X-axis of the range.
     * @param {number} startY   Starting Y-axis of the range.
     * @param {number} endX     Ending X-axis of the range.
     ...
     */
    FIND_AND_REPLACE = (startX, startY, endX, endY, find, replace) => {
        for (let y = startY; y <= endY; y++) {
            for (let x = startX; x <= endX; x++) {
                const cell = this.grid[y][x];
                if (typeof cell.value === 'string') {
                    cell.setValue(cell.value.replace(new RegExp(find, 'g'), replace));
                }
            }
        }
    }
